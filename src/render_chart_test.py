...
def main():
    ...
    candles = api.get_candles('USD_CAD', 'M1', count=20)
...
"""
    ...
    std60 = opy.objects.indicators.SampleStandardDeviation(on='close', period=30, color='orange', name='std 60')
    stdforprice = opy.objects.indicators.SampleStandardDeviation(on='close', period=30, color='orange', name='std 60')
    zscore = opy.objects.indicators.ZScoreOfPrice(stdforprice, on='close', period=30, color='black', name='zscore')
